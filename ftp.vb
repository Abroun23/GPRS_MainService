...
Public Class FTP
    ...
    End Function
    Public Function GetLastModify(ByVal Filename As String) As String
        Dim Path, URI As String
        ...
        URI = Hostname & Path
        fRequest = GetRequest(URI)
        fRequest.Method = WebRequestMethods.Ftp.GetDateTimestamp
        GetResponseString(fRequest) ' Get but Ignore Response
        resp = fRequest.GetResponse
    ...
    ''' <returns>True if Success, False if Error/Fail</returns>
    Public Function Upload(ByVal LocalFileInfo As FileInfo, ByRef er As String, Optional ByVal RemoteFilename As String = "") As Boolean
    ...
    ''' <param name="RemoteFilename">Target Filename (Optional)</param>
    ''' <returns>True if Success, False if Error/Fail</returns>
    Public Function Upload(ByVal LocalFilename As String, Optional ByVal RemoteFilename As String = "") As Boolean
        If Not File.Exists(LocalFilename) Then ' Check Local File Exists
            Throw New ApplicationException("File " & LocalFilename & " not found")
        End If
        Dim er As String = ""

        If Upload(New FileInfo(LocalFilename), er, RemoteFilename) Then
            Upload = True
        Else
            If er <> "" Then
                Upload = False
    ...
    ''' <returns>True if Success, False if Error/Fail</returns>
    Public Function Download(ByVal RemoteFilename As String, ByVal LocalFileInfo As FileInfo, Optional ByVal PermitOverwrite As Boolean = False)
    ...
    ''' <param name="PermitOverwrite">Overwrite Local File if it Exists</param>
    ''' <returns>True if Success, False if Error/Fail</returns>
    Public Function Download(ByVal RemoteFilename As String, ByVal LocalFilename As String, Optional ByVal PermitOverwrite As Boolean = False) As Boolean
